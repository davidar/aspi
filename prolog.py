...
class PrologASPI:
    def __init__(self, args: List[str] = []) -> None:
        ...
        self._display_ldcs = ldcs.LDCS()  # for --> output on deferred definitions
        self._asp = None  # lazy — created on first !/?!/#fluent
        self._asp_args = list(args)
        self._asp_cmd_buffer: List[str] = []  # commands to replay when ASP starts
        self._prolog_cmd_buffer: List[str] = []  # definitions to compile on first ?
    ...
    def _ensure_asp(self) -> 'asp_backend.ASPI':
        """Lazily initialize ASP backend, replaying buffered commands."""
        if self._asp is None:
            ...
            # Replay buffered commands silently
    ...
    def _ensure_prolog(self) -> None:
        """Lazily compile buffered definitions to Prolog."""
    ...
    def _asp_query(self, cmd: str) -> None:
        """Route a query through the ASP backend."""
        import io, contextlib
        buf = io.StringIO()
        ...
        for line in output.split('\n'):
            ...
            if line:
                print(line)
        print()
        # Sync that() facts to Prolog engine for mixed ?!/? sequences
        # Join multiline that: output (continuation lines start with whitespace + |)
        lines = output.split('\n')
        joined = []
        for line in lines:
            stripped = line.strip()
    ...
    def _split_bar_values(self, s: str) -> list:
        """Parse 'v1 | v2 | v3' respecting nested parens."""
        values = []
        depth = 0
        current = []
        for ch in s:
    ...
    def repl(self, cmd: str) -> None:
        ...
        if cmd == 'thanks.':
            ...
            sys.exit(0)
        # ASP-routed queries — ?! suffix
        if cmd.endswith('?!'):
            self._asp_query(cmd[:-1])  # strip !, pass "...?" to ASP
            return
        # Planning goals — delegate to ASP backend entirely
        if cmd.endswith('!') or cmd.startswith('#fluent '):
            self._ensure_asp().repl(cmd)
            return
        # Buffer commands for ASP replay (only mirror if ASP is already active)
        ...
        # Definitions — show ASP -->, buffer for lazy Prolog compilation
        if cmd.endswith('.'):
            ...
            print('understood.\n')
            return
        # Prolog query — compile buffered definitions first
        self._ensure_prolog()
        res = self.eval(cmd)
        if res is not None:
